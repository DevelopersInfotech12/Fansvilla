    ...
    <>
      <style>{`
        ...
        .infl-hero {
          ...
          min-height: 600px;
          overflow: hidden; background: #050a18;
        ...
        .infl-overlay-dark {
          ...
          background: linear-gradient(
            to right,
            rgba(5,10,24,0.97) 0%,
            rgba(5,10,24,0.82) 45%,
            rgba(5,10,24,0.45) 70%,
        ...
        /* 
         * FIX SS2: content was padding: 0 7% — frame (right side) overlapped on mid screens.
         * Fix: content max-width capped to 55% on desktop, 100% on mobile.
         * Frame hidden below md breakpoint so it never overlaps content.
         */
        .infl-content {
        ...
        /* On narrow screens content takes full width */
        @media (max-width: 900px) {
          .infl-content { max-width: 100%; padding: 80px 5% 200px; }
          .infl-frame   { display: none !important; }
          /* hide bot-right card on narrow — overlaps stats */
          .infl-card.bot-right { display: none !important; }
        }
        @media (max-width: 600px) {
          .infl-content { padding: 70px 5% 220px; }
          /* hide all floating cards on small mobile */
          .infl-card { display: none !important; }
          .infl-title { font-size: clamp(52px, 14vw, 100px) !important; }
          .infl-subtitle { font-size: clamp(38px, 10vw, 80px) !important; }
        }
        ...
        .infl-tag {
          ...
          margin-bottom: 24px; backdrop-filter: blur(12px); border: 1px solid;
        ...
        .infl-btns { display: flex; gap: 12px; align-items: center; margin-bottom: 40px; flex-wrap: wrap; }
        .infl-btn-primary {
        ...
        .infl-btn-ghost {
          padding: 12px 26px; border-radius: 4px; background: transparent;
          font-family: 'DM Sans', sans-serif; font-weight: 600; font-size: 13px;
          letter-spacing: 0.5px; cursor: pointer;
        ...
        /* FIX SS2: stats had no bottom padding — overlapped nav bar */
        .infl-stats { display: flex; gap: 0; flex-wrap: wrap; }
        .infl-stat {
          padding: 0 24px 0 0; margin-right: 24px;
          border-right: 1px solid rgba(255,255,255,0.12);
        }
        .infl-stat:last-child { border-right: none; padding-right: 0; margin-right: 0; }
        .infl-stat-n {
          font-family: 'Bebas Neue', cursive; font-size: 32px; line-height: 1;
          transition: color 0.5s ease;
        }
        .infl-stat-l { font-size: 10px; font-weight: 500; letter-spacing: 2px; text-transform: uppercase; color: rgba(255,255,255,0.4); margin-top: 2px; }

        /* Right frame — hidden on mobile via media query above */
        .infl-frame {
          position: absolute; right: 5%; top: 50%; transform: translateY(-50%);
          width: clamp(200px, 24vw, 380px); z-index: 8;
        ...
        .infl-frame-label {
          ...
          font-weight: 600; text-align: center; transition: color 0.5s ease;
        }
        ...
        .infl-nav-dot { width:5px; height:5px; border-radius:50%; opacity:0; transition:opacity 0.3s; flex-shrink: 0; }
        ...
        .infl-arrow {
          ...
          /* FIX SS2: top:50% minus half of nav (80px) and half arrow (24px) */
          top: calc(50% - 40px - 0px);
        ...
        .infl-arrow.left  { left: 16px; }
        .infl-arrow.right { right: 16px; }

        /* Floating influencer cards */
        .infl-card {
          position: absolute; z-index: 15;
          background: rgba(10,16,36,0.88); backdrop-filter: blur(16px);
          border: 1px solid rgba(255,255,255,0.1); border-radius: 8px;
          padding: 10px 14px; display: flex; align-items: center; gap: 10px;
          animation: floatCard 4s ease-in-out infinite;
          min-width: 155px; max-width: 200px;
        }
        /* FIX SS2: cards repositioned so they don't overlap content or each other */
        .infl-card.top-left  { top: 10%; left: 2%; animation-delay: 0s; }
        /* bot-left moved up so it clears stats bar (80px nav + some breathing room) */
        .infl-card.bot-left  { bottom: 110px; left: 2%; animation-delay: 1.5s; }
        /* bot-right hidden on <=900px, on desktop keep right side */
        .infl-card.bot-right { bottom: 110px; right: 2%; animation-delay: 0.8s; }
        @keyframes floatCard { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-8px)} }
        .infl-card-avatar { width:34px; height:34px; border-radius:50%; object-fit:cover; flex-shrink:0; }
        ...
        .infl-card-live   { width:6px; height:6px; border-radius:50%; background:#00FF94; margin-left:auto; box-shadow:0 0 6px #00FF94; flex-shrink:0; }

        /* Season badge */
        .infl-season-badge {
          position: absolute; top: 20px; left: 50%; transform: translateX(-50%);
          z-index: 20; display: flex; align-items: center; gap: 8px;
          background: rgba(10,16,36,0.75); backdrop-filter: blur(16px);
      ...
      <div className="infl-hero">
        ...
        {/* Banner image */}
        <img key={s.id} src={s.image} alt={s.title} className={`infl-img ${transitioning ? 'out' : 'in'}`} />
        ...
        <div className="infl-season-badge">
          <span style={{ width:6, height:6, borderRadius:'50%', background:s.accent, display:'inline-block', boxShadow:`0 0 8px ${s.accent}`, flexShrink:0 }} />
          {s.tag.replace(/^[^ ]+ /, '')}
        </div>
        ...
        {/* Right frame — hidden on <=900px */}
        <div className="infl-frame" style={{ opacity: transitioning ? 0 : 1 }}>
        ...
        <div className="infl-nav">
          {SLIDES.map((sl, i) => (
            <button key={sl.id} className={`infl-nav-item ${i === current ? 'active' : ''}`} onClick={() => goTo(i)}>
              ...
              {i === current && (
                <div key={current} className="infl-progress running" style={{ background:sl.accent }} />
              )}
            </button>
          ))}
        </div>
